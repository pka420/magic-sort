"""
The leaderboard, one per level: a player's best on each level, ranked.
...
but the table only ever keeps a player's best per level, and only players whose
email is verified and whose name is set appear on a board.
"""
...
def leaderboard(level_id: int, db: Session = Depends(get_db)):
    rows = db.execute(
...
def submit_score(
    ...
    score = db.get(Score, (user.id, request.level_id))
    if score is None:
        score = Score(user_id=user.id, level_id=request.level_id, total=request.total)
        db.add(score)
